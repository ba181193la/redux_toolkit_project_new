...
import { useDeleteDepartmentMutation } from '../../../../redux/RTK/departmentMasterApi';
...
const DepartmentDataTable = ({
  ...
  //* RTK Queries
  const [triggerDeleteDepartment] = useDeleteDepartmentMutation();

  //* Handlers for pagination
  const handleOnPageChange = (event, newPage) => {
    dispatch(setDepartmentPageIndex(newPage));
  };

  const handleOnPageSizeChange = (event) => {
    dispatch(setDepartmentPageSize(parseInt(event.target.value, 10)));
    dispatch(setDepartmentPageIndex(0));
  };

  //* Edit department
  const editDepartment = (deptId) => {
    setSelectedDepartmentId(deptId);
    setShowDepartmentModal(true);
  };

  //* Delete Department
  const deleteDepartment = async (deptDetails) => {
    const callback = async () => {
      try {
        await triggerDeleteDepartment({
        ...
        showSweetAlert({
          type: 'success',
          title: 'Deleted!',
          text: `${getlabel('MM_DM_Department', labels, i18n.language)} has been deleted.`,
          timer: 3000,
          gif: SuccessGif,
          height: 250,
          width: 250,
              ...
              {sortedRecords?.length > 0 ? (
                            ...
                            {checkAccess(isSuperAdmin, isView, isEdit) && (
                              <Tooltip title="Edit" arrow>
                                <StyledImage
                                  ...
                                  alt="Edit"
                                  onClick={() =>
                                    editDepartment(row.DepartmentId)
                                  }
                                />
                              </Tooltip>
                            )}
                ...
                <TableBody>
                  <StyledTableRow>
                    <StyledTableBodyCell
                      colSpan={
                        columns?.filter((col) => col.isSelected && col.isShow).length + 2
                      }
                          ...
                          {checkAccess(isSuperAdmin, isView, isEdit) && (
                            <Tooltip title="Edit" arrow>
                              <StyledImage
                                ...
                                src={EditIcon}
                                alt="Edit"
                                onClick={() => editDepartment(row.DepartmentId)}
                              />
                            </Tooltip>
                          )}
                          {checkAccess(isSuperAdmin, isView, isDelete) && (
