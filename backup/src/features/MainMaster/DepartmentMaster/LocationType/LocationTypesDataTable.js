...
import { useDeleteLocationTypeMutation } from '../../../../redux/RTK/departmentMasterApi';
...
const LocationTypesDataTable = ({
  ...
  //* RTK Queries
  const [triggerDeleteLocationType] = useDeleteLocationTypeMutation();

  //* Handlers for pagination
  const handleOnPageChange = (event, newPage) => {
    dispatch(setLocationTypePageIndex(newPage));
  };

  const handleOnPageSizeChange = (event) => {
    dispatch(setLocationTypePageSize(parseInt(event.target.value, 10)));
    dispatch(setLocationTypePageIndex(0));
  };

  //* Edit department
  const editLocationType = (locationId) => {
    setSelectedLocationId(locationId);
    setShowLocationTypeModal(true);
  };

  //* Delete Department
  const deleteLocationType = async (locationDetails) => {
    const callback = async () => {
      try {
        await triggerDeleteLocationType({
        ...
        showSweetAlert({
          type: 'success',
          title: 'Deleted!',
          text: `${getlabel('MM_DM_LocationType', labels, i18n.language)} has been deleted.`,
          timer: 3000,
          gif: SuccessGif,
          height: 250,
          width: 250,
                            ...
                            {checkAccess(isSuperAdmin, isView, isEdit) && (
                              <Tooltip title="Edit" arrow>
                                <StyledImage
                                  ...
                                  alt="Edit"
                                  onClick={() =>
                                    editLocationType(row.LocationId)
                                  }
                                />
                              </Tooltip>
                            )}
                            {checkAccess(isSuperAdmin, isView, isDelete) && (
                              <Tooltip title="Delete" arrow>
                                <StyledImage
                                  ...
                                  src={DeleteIcon}
                                  alt="Delete"
                                  onClick={() => deleteLocationType(row)}
                                />
                              </Tooltip>
                            )}
                          </FlexContainer>
                          ...
                          {checkAccess(isSuperAdmin, isView, isEdit) && (
                            <Tooltip title="Edit" arrow>
                              <StyledImage
                                ...
                                src={EditIcon}
                                alt="Edit"
                                onClick={() => editLocationType(row.LocationId)}
                              />
                            </Tooltip>
                          )}
                          {checkAccess(isSuperAdmin, isView, isDelete) && (
...
export default LocationTypesDataTable;
