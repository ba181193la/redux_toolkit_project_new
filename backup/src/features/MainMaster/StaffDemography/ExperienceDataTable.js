...
const ExperienceDataTable = ({
  ...
  const pageFields = fieldAccess?.Data?.Sections?.find(
    (sectionName) => sectionName?.SectionName === 'Experience'
  )?.Fields;
  ...
  const educationColumnsList = [
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_Exp_Title')?.IsShow,
    },
    {
      id: 'organization',
      translationId: 'MM_SF_Organization',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_Exp_Organization')
        ?.IsShow,
    },

    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_Exp_Country')?.IsShow,
    },
    {
      id: 'fromDate',
      translationId: 'MM_SF_FromDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_Exp_FromDate')?.IsShow,
    },
    {
      id: 'toDate',
    ...
    },
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_Exp_SubmittedBy')
        ?.IsShow,
    },
    {
      id: 'submittedDate',
      translationId: 'MM_SF_SubmittedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_Exp_SubmittedDate')
        ?.IsShow,
    },
  ];
  ...
  //* Add more data
  const addMore = () => {
    const lastRecord = experienceRecords[experienceRecords?.length - 1];
    const unsavedRecord = experienceRecords?.findIndex(
      (record) => record?.isEdit === true
    );
    if (
      !lastRecord?.title &&
      !lastRecord?.organization &&
      !lastRecord?.country &&
      !lastRecord?.fromDate &&
      !lastRecord?.toDate &&
      ...
      showToastAlert({
        type: 'custom_info',
        text: `Enter records in Exoerience Tab row no ${experienceRecords?.length}`,
        gif: 'InfoGif',
      });
  ...
  const changeRowDetails = (rowIndex, column, value) => {
    const updatedRecords = [...experienceRecords];
    updatedRecords[rowIndex] = { ...updatedRecords[rowIndex], [column]: value };
    setExperienceRecords(updatedRecords);
  };

  //* Delete row details
  const deleteRecord = (index) => {
    const records = [...experienceRecords];
    setExperienceRecords(records.filter((e, i) => i !== index));
  };
  ...
  return (
    <FlexContainer flexDirection="column" width="100%">
      <TableContainer component={Paper} style={{ border: '1px solid #d20962' }}>
        <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
          <StyledTableHead backgroundColor="#d20962" borderLeft="none">
          ...
          {experienceRecords?.length > 0 ? (
            <TableBody>
              {experienceRecords?.map((row, rowIndex) => (
