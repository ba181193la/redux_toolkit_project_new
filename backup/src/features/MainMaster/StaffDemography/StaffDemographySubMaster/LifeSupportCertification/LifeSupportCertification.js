...
import AddEditLifeSupportCertification from './AddEditLifeSupportCertification';
import {
...
const LifeSupportCertification = ({ labels, fieldAccess }) => {
  //* Hooks declaration
  ...
  const { pageIndex, pageSize } = useSelector(
    (state) => state?.staffDemography?.lifeSupportCertification
  );

  //* State variables
  const [showLSCModal, setShowLSCModal] = useState(false);
  const [selectedLSCId, setSelectedLSCId] = useState(null);
  ...
  const [lscList, setLSCList] = useState([]);
  const [filteredFacility, setFilteredFacility] = useState(null);

  // //* Handlers for pagination
  const handleOnPageChange = (event, newPage) => {
    dispatch(setLifeSupportCertificationPageIndex(newPage));
  };

  const handleOnPageSizeChange = (event) => {
    dispatch(
      setLifeSupportCertificationPageSize(parseInt(event?.target?.value, 10))
    );
    dispatch(setLifeSupportCertificationPageIndex(0));
  };
  ...
  const orderedLSCList = [...lscList].sort((a, b) => {
  ...
  const tableColumnList = [
    {
      id: 'Facility',
      translationId: 'MM_SF_LSC_Facility',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_LSC_Facility')?.IsShow,
    },
    {
      id: 'CertificationTitle',
      translationId: 'MM_SF_LSC_Certification',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_LSC_Certification')
        ?.IsShow,
    },
    {
      id: 'StaffCategory',
      translationId: 'MM_SF_LSC_StaffCategory',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_LSC_StaffCategory')
        ?.IsShow,
    },
    {
      id: 'CreatedByName',
      translationId: 'MM_SF_LSC_AddedEditedBy',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_LSC_AddedEditedBy')
        ?.IsShow,
    },
    {
      id: 'Added_EditedDate',
      translationId: 'MM_SF_LSC_AddedEditedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_LSC_AddedEditedDate')
        ?.IsShow,
    },
  ];
  ...
  //* Edit tab details
  const editLSC = (tabId) => {
    setSelectedLSCId(tabId);
    setShowLSCModal(true);
  };
  ...
  return (
    <Accordion sx={{ border: '5px', borderColor: '#0083c0' }} expanded={true}>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Label
                value={getlabel(
                  fieldLabels?.find(
                    (x) => x.TranslationId === 'MM_SF_LSC_Facility'
                  )?.TranslationId,
                  {
                    Data: fieldLabels,
          ...
          <FlexContainer flexDirection="column" width="100%" padding="10px">
            ...
            >
              <CustomScrollbars
                style={{
                  height: orderedLSCList?.length > 0 ? '350px' : '85px',
                }}
              ...
              >
                {false ? (
                  ...
                  <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
                    ...
                    {orderedLSCList?.length > 0 ? (
                      <TableBody>
                        {orderedLSCList?.map((row, rowIndex) => (
        ...
        {showLSCModal && (
          <AddEditLifeSupportCertification
            showLSCModal={showLSCModal}
            setShowLSCModal={setShowLSCModal}
            selectedLSCId={selectedLSCId}
            setSelectedLSCId={setSelectedLSCId}
            labels={labels}
            refetch={refetch}
...
export default LifeSupportCertification;
