...
const TrainingDataTable = ({
  ...
  const pageFields = fieldAccess?.Data?.Sections?.find(
    (sectionName) => sectionName?.SectionName === 'Training'
  )?.Fields;
  ...
  const educationColumnsList = [
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_T_Title')?.IsShow,
    },
    {
      id: 'institution',
      translationId: 'MM_SF_IssueAuthorityInstitution',
      isShow: pageFields?.find(
        (x) => x.FieldId === 'SF_T_IssueAuthorityInstitution'
      )?.IsShow,
    },

    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_T_Country')?.IsShow,
    },
    {
      id: 'fromDate',
      translationId: 'MM_SF_FromDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_T_FromDate')?.IsShow,
    },
    {
      id: 'toDate',
      translationId: 'MM_SF_ToDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_T_ToDate')?.IsShow,
    },
    {
      id: 'toDate',
      translationId: 'MM_SF_ToDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_ToDate')?.IsShow,
    },
    {
      id: 'certificate',
    ...
    },
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_T_SubmittedBy')?.IsShow,
    },
    {
      id: 'submittedDate',
      translationId: 'MM_SF_SubmittedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_T_SubmittedDate')
        ?.IsShow,
    },
  ];
  ...
  const addMore = () => {
    ...
    const unsavedRecord = trainingRecords?.findIndex(
    ...
    if (
      ...
      showToastAlert({
        type: 'custom_info',
        text: `Enter records in Education Tab row no ${trainingRecords?.length}`,
        gif: 'InfoGif',
      });
  ...
  const changeRowDetails = (rowIndex, column, value) => {
    const updatedRecords = [...trainingRecords];
  ...
  const deleteRecord = (index) => {
    ...
    setTrainingRecords(records.filter((e, i) => i !== index));
  };
  ...
  return (
    <FlexContainer flexDirection="column" width="100%">
      <TableContainer component={Paper} style={{ border: '1px solid #f47721' }}>
        <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
          <StyledTableHead backgroundColor="#f47721" borderLeft="none">
          ...
          {trainingRecords?.length > 0 ? (
            <TableBody>
              {trainingRecords?.map((row, rowIndex) => (
