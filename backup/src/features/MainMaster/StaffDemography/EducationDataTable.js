...
const EducationDataTable = ({
  ...
  const pageFields = fieldAccess?.Data?.Sections?.find(
    (sectionName) => sectionName?.SectionName === 'Education'
  )?.Fields;
  ...
  const educationColumnsList = [
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_Title')?.IsShow,
    },
    {
      id: 'institution',
      translationId: 'MM_SF_Institution',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_Institution')
        ?.IsShow,
    },

    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_Country')?.IsShow,
    },
    {
      id: 'fromDate',
      translationId: 'MM_SF_FromDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_FromDate')?.IsShow,
    },
    {
      id: 'toDate',
    ...
    },
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_SubmittedBy')
        ?.IsShow,
    },
    {
      id: 'submittedDate',
      translationId: 'MM_SF_SubmittedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_EDU_SubmittedDate')
        ?.IsShow,
    },
  ];
  ...
  const addMore = () => {
    ...
    const unsavedRecord = educationRecords?.findIndex(
    ...
    if (
      ...
      showToastAlert({
        type: 'custom_info',
        text: `Enter records in Education Tab row no ${educationRecords?.length}`,
        gif: 'InfoGif',
      });
  ...
  const deleteRecord = (index) => {
    const records = [...educationRecords];
    setEducationRecords(records.filter((e, i) => i !== index));
  };
  ...
  return (
    <FlexContainer flexDirection="column" width="100%">
      <TableContainer component={Paper} style={{ border: '1px solid #7d3f98' }}>
        <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
          <StyledTableHead backgroundColor="#7d3f98" borderLeft="none">
          ...
          {educationRecords?.length > 0 ? (
            <TableBody>
              {educationRecords?.map((row, rowIndex) => (
