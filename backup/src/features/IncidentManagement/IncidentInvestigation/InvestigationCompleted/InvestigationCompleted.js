...
import InvestigationCompletedData from './InvestigationCompletedData';
import DataTableCompleted from './DataTableCompleted';
import FilterFormFields from './FilterFormCompletedFields';
import {
  FlexContainer,
  StyledButton,
  StyledImage,
...
const InvestigationCompleted = () => {
  const [seletedCompletedColumns, setSelectedCompletedColumns] = useState(
    InvestigationCompletedData
  );
  ...
  const { filters, isFilterApplied } = useSelector(
   ...
   const [triggerDownloadData] = useGetDownloadDataInvestigationCompleteMutation();
     ...
     const handleOnDownloadOrPrint = async (fileType) => {
       if (filteredRecords?.length > 0) { 
         try {
           ...
           if (fileType === 'Pdf') {
              saveAs(blob, 'Incident Investigation Completed.pdf');
             setMessage('Download completed');
             setTimeout(() => {
               setIsLoading(false);
             }, 2000);
           } else {
             saveAs(blob, 'Incident Investigation Completed.xlsx');
             setMessage('Download completed');
             setTimeout(() => {
               setIsLoading(false);
             }, 2000);
  ...
  const {
    data: getIncidentInvestigationCompleted = [],
    isFetching,
    refetch,
  } = useGetIncidentInvestigationCompletedQuery(
    {
  ...
  const { totalRecords, records } = getIncidentInvestigationCompleted || {
    totalRecords: 0,
    records: [],
  };
...
export default InvestigationCompleted;
