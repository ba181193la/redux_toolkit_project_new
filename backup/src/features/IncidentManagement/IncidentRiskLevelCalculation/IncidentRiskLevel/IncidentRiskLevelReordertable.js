...
import { useGetCategoryDetailsQuery, useGetConsequencelevelDetailsQuery, useGetIncidentRiskDetailsQuery, useGetLikelihoodDetailsQuery, useReorderCategoryAffectedMutation, useReorderConsequenceLevelMutation, useReorderIncidentriskLevelMutation, useReorderliklihoodMutation, useUpdateConsequenceLevelMutation } from '../../../../redux/RTK/incidentRiskLevelApi';
...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'IncidentRiskLevel') && (
        <StyledTableBodyCell>{item?.IncidentRiskLevel}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'ActionToBeTaken') && (
        <StyledTableBodyCell>{item?.ActionToBeTaken}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'ScoreRange') && (
        <StyledTableBodyCell>{item?.ScoreRange}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const {
  ...
  useEffect(() => {
    if (getIncidentData?.Records) {
      setTableData(getIncidentData?.Records);
    }
  }, [getIncidentData]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      incidentRiskLevelId: item.IncidentRiskLevelId,
      incidentRiskLevel: item.IncidentRiskLevel,
      actionToBeTaken: item.ActionToBeTaken,
      greaterScoreRange: item.GreaterScoreRange,
      lesserScoreRange: item.LesserScoreRange,
      isRCARequired: item.IsRCARequired,
