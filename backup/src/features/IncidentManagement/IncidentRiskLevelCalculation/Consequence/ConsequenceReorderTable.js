...
import { useGetCategoryDetailsQuery, useGetConsequenceDetailsQuery, useGetConsequencelevelDetailsQuery, useGetIncidentRiskDetailsQuery, useGetLikelihoodDetailsQuery, useReorderCategoryAffectedMutation, useReorderConsequenceLevelMutation, useReorderConsequenceMutation, useReorderIncidentriskLevelMutation, useReorderliklihoodMutation, useUpdateConsequenceLevelMutation } from '../../../../redux/RTK/incidentRiskLevelApi';
...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'ConsequenceLevel') && (
        <StyledTableBodyCell>{item?.ConsequenceLevel}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'Consequence') && (
        <StyledTableBodyCell>{item?.Consequence}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const {
  ...
  useEffect(() => {
    if (consequenceData?.Records) {
      setTableData(consequenceData?.Records);
    }
  }, [consequenceData]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      ...
      consequenceLevelId: item.ConsequenceLevelId,
      categoryAffected: item.CategoryAffected,
