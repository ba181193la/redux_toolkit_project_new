...
const IncidentRiskTable_Cl = ({ getConseqenceLevelData }) => {
    console.log("consequenceLevel_Print", getConseqenceLevelData)
  return (
    <Box sx={{ display: "flex", justifyContent: "center", padding: 2 }}>
      <TableContainer component={Paper} sx={{ maxWidth: 800 }}>
        ...
        <Table>
          ...
          {/* Table Body */}
          <TableBody>
            {getConseqenceLevelData?.Records?.map((item, index) => (
              <TableRow key={index}>
                <TableCell align="center" sx={{ border: "1px solid black" }}>{index + 1}</TableCell>
                <TableCell align="center" sx={{ border: "1px solid black" }}>{item.ConseqenceLevel || "N/A"}</TableCell>
