...
const ApproveIncidentTable = ({ labels, columns, data }) => {
    ...
    <StyledTableRow key={index}>
      ...
      {columns.map((column) => (
        <StyledTableBodyCell key={column.name}>
          {['ReportedDate', 'IncidentDate', 'AssignOn', 'RequestedDate', 'EventDate', 'ResponseDate', 'TargetDate'].includes(column.name) && row[column.name] ? (
            formatDate(row[column.name])
          ) : column.name === 'EventTime' && row[column.name] ? (
            row[column.name].slice(0, 5) 
          ) : (
            row[column.name] ?? ''
          )}
        </StyledTableBodyCell>
