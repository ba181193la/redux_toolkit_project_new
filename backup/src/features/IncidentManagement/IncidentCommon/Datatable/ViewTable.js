...
const ViewTable = ({ labels, columns, data }) => {
              ...
              data.map((row, index) => (
                <StyledTableRow key={index}>
                  ...
                  {columns.map((column) => (
                     ...
                     {['TargetDate', 'IncidentDate', 'EventDate', 'ResponseDate', 'RequestedDate'].includes(column.name) && row[column.name]
                       ? formatDate(row[column.name])
                       : column.name === 'EventTime' && row[column.name] 
                         ? row[column.name].split('.')[0].slice(0, 5) 
                         : (row[column.name] ?? '')}
                   </StyledTableBodyCell>
                  ))}
                </StyledTableRow>
