...
import VisitorTable from './Datatable/II_PL_VisitorTable';
import WitnessTable from './Datatable/II_PL_WitnessTable';
...
// import IncidentHistory from './Datatable/II_PL_IncidentHistory'
import NotificationHistory from './Datatable/II_PL_NotificationHistory';
...
const IncidentDetails = ({ isDelete = false }) => {
    ...
    useGetIncidentDetailsPendingByIdQuery(
      {
        ...
        incidentId: id,
        moduleId: 2,
        IsDeletedIncident: Boolean(isDelete),
      },
      { skip: !id }
    );
  ...
  const mainCategoryFormValueDataSafe =
    incidentFormBuilderData?.mainCategoryFormValue || [];
    console.log('questionsData', questionsData);
    console.log('mainCategoryFormValueDataSafe', mainCategoryFormValueDataSafe);
  const subCategoryFormValueDataSafe =
    incidentFormBuilderData?.subCategoryFormValue || [];
  const incidentFormValueDataSafe =
    incidentFormBuilderData?.incidentDetailFormValue || [];
  ...
  const fieldsConfig = [
    ...
    },
    {
      ...
      translationId: 'IM_IR_ReportingEmployeeId',
      label: 'Reported Time',
      name: 'ReportingStaffFacilityId',
    },
    {
      fieldId: `RI_P_ReportingStaffName`,
      translationId: 'IM_RI_ReportingStaffName',
    ...
    },
    {
      ...
      label: 'StaffCategory',
      name: 'ReportingEmployeeStaffCategory',
    },
    {
  ...
  useEffect(() => {
    ...
    if (
      reportingDateTimeValue &&
      reportingDateTimeValue !== incidentReportDateTime
    ) {
        ...
        <AccordionDetails>
            ...
            {configs?.fieldsConfig.map((fieldConfig) => {
              ...
              const getFormattedValue = (value, name) => {
                ...
                if (name === 'IncidentTime' && incidentDateTime) {
                  const timePart = incidentDateTime.split('T')[1];
                  const [hours, minutes] = timePart.split(':');
                  return `${hours}:${minutes}`;
                }

                if (name === 'ReportTime' && incidentReportDateTime) {
                  const timePart = incidentReportDateTime.split('T')[1];
                  const [hours, minutes] = timePart.split(':');
                  return `${hours}:${minutes}`;
                }
              ...
              return (
                <Grid
                  item
                  xs={fieldConfig.name === 'ReportingStaffFacilityId' ? 12 : 4}
          ...
          <Grid container spacing={2} p={0}>
            {configs?.incidentConfig?.map((fieldConfig) => {
              const fieldValue = reportIncidentSafe[fieldConfig.name] ?? '';
              return (
                <Grid item xs={12} key={fieldConfig.fieldId}>
          ...
          <Grid container spacing={2} p={0} mt={2}>
            {configs?.newRow?.map((fieldConfig) => {
              const fieldValue = reportIncidentSafe[fieldConfig.name] ?? '';
              return (
                <Grid item xs={12} key={fieldConfig.fieldId}>
                  <Grid container alignItems="center">
          ...
          {incidentData?.Data?.CompanionInvolved && (
            <>
              <FormLabel style={{ margin: '10px 0px 10px 0px' }}>
                {getlabel(
          ...
          {incidentOutsourceInvolvedSafe?.length > 0 && (
            <>
              ...
              <FormLabel style={{ margin: '10px 0px 10px 0px' }}>
                {getlabel(
                  titleConfig.find(
                    (config) => config.name === 'OutsourcedStaff'
                  )?.translationId,
                  filterLabels,
                  i18n.language
                )}
