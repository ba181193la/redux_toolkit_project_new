...
const Patient = ({ labels, patientTabChecked, id }) => {
  ...
  const { incidentPatientInvolved, IncidentData, pageLoadData, staffDetails } =
      ...
      const updateConfig = configData.map((item) => {
        ...
        if (item.name === 'isPhysicianNotified') {
          return {
            ...item,
            options:
              pageLoadData?.Data?.IsPhysicianNotifiedList?.map((item) => {
                return {
                  text: item.Status,
                  value: item.Status,
                };
        ...
        if (item.name === 'gender') {
          return {
            ...item,
            options:
              pageLoadData?.Data?.GenderList?.map((gender) => ({
                text: gender.Gender,
                value: gender.Gender,
              })) || [],
          };
  ...
  useEffect(() => {
    if (
      values?.isPhysicianNotified === 'Yes' ||
      values?.isPhysicianNotified === 'No'
    ) {
      const newFieldConfigData = configData?.map((item) =>
        item.isPhysicianNotified === 'Yes'
          ? {
              ...item,
              isFieldShow: values?.isPhysicianNotified === 'Yes' ? true : false,
            }
          : item
      );
  ...
  const handlesavePatients = () => {
    try {
        ...
        ? incidentPatientInvolved.map((item) => {
            if (item.patientId === values?.patientId) {
              return {
                ...
                physicianName:
                  values?.physicianName || item.physicianName || '',
                isPhysicianNotified:
                  values?.isPhysicianNotified || item.isPhysicianNotified,
                NotifiedPhysician:
                  values?.NotifiedPhysician || item.NotifiedPhysician,
                physicianUserId:
                  values?.notifiedPhysicianUserId ||
                  item.notifiedPhysicianUserId,
                notifiedPhysicianUserId:
                  values?.notifiedPhysicianUserId ||
                  item.notifiedPhysicianUserId,
                departmentId: values?.PhysicianDepartment || item.departmentId,
                PhysicianDepartmentName:
                  values?.PhysicianDepartmentName ||
                  item.PhysicianDepartmentName,
                isEditing: false,
                isSaved: true,
                isDelete: false,
              };
      ...
      if (!updatedRows.find((item) => item.patientId === values?.patientId)) {
        ...
        if (
          (!incidentPatientInvolved || incidentPatientInvolved?.length === 0) &&
  ...
  const getGridColumns = (fieldConfig, gridWindow) => {
    if (gridWindow === 'md' || gridWindow === 'lg') {
      ...
      if (fieldConfig.grid === 5) {
        if (values?.isPhysicianNotified === 'Yes') return 4;
        else return 6;
      } else return 12;
    }
  };
    ...
    >
      ...
      <FormContainer>
        <Grid container spacing={2} display={'flex'}>
          {configData
            ?.filter((item) => item.isFieldShow == true)
            ?.map((fieldConfig) => {
