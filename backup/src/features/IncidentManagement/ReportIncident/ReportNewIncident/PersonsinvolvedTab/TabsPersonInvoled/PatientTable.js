...
const PatientTable = ({ IncidentData, setHideSaveBtn }) => {
  const [selectedIndex, setSelectedIndex] = useState(null);
  const { values, setFieldValue } = useFormikContext();
  const dispatch = useDispatch();
  const { id } = useParams();
  const { incidentPatientInvolved, pageLoadData } = useSelector(
    (state) => state.reportIncident
  );

  const handleOnDelete = (patientId, index) => {
    if (id) {
      const checkNewPatient = incidentPatientInvolved?.find(
        (item) => item.patientId === patientId && item.isNewRow == true
      );
      if (!checkNewPatient) {
              ...
              incidentPatientInvolved
                ...
                ?.map((row, index) => {
                  const departmentName =
                    pageLoadData?.Data?.DepartmentList?.find(
                      (item) => item.DepartmentId == row?.departmentId
                    )?.DepartmentName;
                  return (
                    <StyledTableRow key={index}>
                      ...
                      <StyledTableBodyCell>
                        <ReadOnlyText name="diagonosis">
                          {row.diagnosis}{' '}
                        </ReadOnlyText>
                      </StyledTableBodyCell>
                      <StyledTableBodyCell>
                        <ReadOnlyText name="visitId">
                          {row.visitId}
                        </ReadOnlyText>
                      </StyledTableBodyCell>
                      <StyledTableBodyCell>
                        ...
                        <FlexContainer className="action-icons">
                          ...
                          <Tooltip title="Delete" arrow>
                            <StyledImage
                              ...
                              alt="Delete"
                              onClick={() =>
                                handleOnDelete(row?.patientId, index)
                              }
                            />
                          </Tooltip>
                        </FlexContainer>
