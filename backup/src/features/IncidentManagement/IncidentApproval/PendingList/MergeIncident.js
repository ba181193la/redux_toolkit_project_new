...
const MergeIncident = ({ entryData, common_data }) => {
  ...
  const [triggerUpdateMergeIncident] = useUpdateMergeIncidentMutation();
  ...
  let ReoccurrenceLog =
    entryData?.reoccurrenceDetails?.length > 0
      ? {
      ...
      const requiredFields = ["mainCategory", "subCategory", "incidentDetail"];


  const handleSubmit = async (values) => {

    const missingFields = requiredFields.filter(field => !selectedIncident?.[field]);
      ...
      const response = await triggerUpdateMergeIncident({
        payload: {
          ...
          incidentApproval_ReoccurrenceLog:
            common_data.incidentApproval_ReoccurrenceLog,
