...
import { useGetReportIncidentPageloadQuery } from '../../../../redux/RTK/IncidentManagement/incidentApprovalApi';
...
const Incidentcategorylist = ({
  ...
  const { data: incidentLits = [], isFetching: isIncidentFetching } = useGetReportIncidentPageloadQuery({
                ...
                {currentItems.length > 0 ? (
                  currentItems.map((incident, index) => (
                    <StyledTableRow key={incident.IncidentNo} onClick={() => handleRowClick(incident)}>
                      ...
                      <StyledTableBodyCell>{incident.SubCategory}</StyledTableBodyCell>
