...
const Incidentcategorylist = ({ onSelectRow, staffFetching }) => {
  const { t } = useTranslation();
  const { selectedFacility, userDetails, selectedModuleId, selectedMenu } =
    useSelector((state) => state.auth);

  const { data: incidentLits = [], isFetching: isIncidentFetching } =
  ...
  const TotalItemss = incidentLits?.Data?.IncidentCategoryList;

  console.log(TotalItemss,"qqqqqqqqq")


  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentItems =
    incidentLits?.Data?.IncidentCategoryList?.slice(
      startIndex,
