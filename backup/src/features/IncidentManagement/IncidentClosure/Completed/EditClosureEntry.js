...
const EditClosureEntry = () => {
  ...
  const [triggerEditClosure] = useEditClosureMutation();
...
useEffect(() => {
  if (entryData?.allQuestionMetaData) {
    setMainCategoryFormValueData(entryData.allQuestionMetaData.mainCategoryFormValueData || []);
    setSubCategoryFormValueData(entryData.allQuestionMetaData.subCategoryFormValueData || []);
    setIncidentDetailFormValueData(entryData.allQuestionMetaData.incidentDetailFormValueData || []);
  }
}, [entryData]);
...
const handleMainCategoryChange = ({ id, value, allValues }) => {
  console.log('Changed Field:', id, 'New Value:', value);
  const formatted = Object.entries(allValues).map(([id, value]) => ({
    id,
    value,
  }));
  console.log('Updated All Values:', formatted);
...
const handleIncidentDetailChange = ({ id, value, allValues }) => {
  ...
  const {
    data: getIncidentClosurePending = [],
    isFetching,
    refetch,
  } = useGetIncidentClosurePendingQuery(
    {
  ...
  // API call using your hook
  const { data: getIncidentLevelData = [], error } =
    useGetIncidentLevelDataQuery({
      payload: {
        headerFacilityId: 2,
  ...
  useEffect(() => {
    if (shouldFetchData && !error) {
      setIncidentLevelData(getIncidentLevelData?.Data || []);
      setShowContainer(true);
    }
  }, [shouldFetchData, error, getIncidentLevelData]);
  ...
  const handleSaveRCASubmit = async (values) => {
    ...
    try {
      ...
      const response = await triggerEditClosure({ formData });
                ...
                <FlexContainer flexDirection={'column'}>
                  <Grid container spacing={2} p={2} style={{ padding: '0px' }}>
                    ...
                    {[
                      ...
                      ['IncidentDetail', 'IncidentDetailsCode'],
                    ...
                    <Grid item xs={12} mt={2}>
                     ...
                     {questionsData?.length > 0 && (
                                          <Grid item xs={12} mt={2}>
                                            ...
                                            <Box mb={2}>
                                              <DynamicFormBuilder formData={questionsData} 
                    ...
                    <Grid container spacing={1}>
                      {filteredTitleConfig?.length > 0 &&
                        ['LessonsLearned', 'IncidentReason/RootCause'].map(
                          (name) => {
                            const field = filteredTitleConfig.find(
                              (f) => f.name === name
                            );
                            console.log(
                              'Debug - Field check for:',
                              name,
                              field
                            );
...
export default EditClosureEntry;
