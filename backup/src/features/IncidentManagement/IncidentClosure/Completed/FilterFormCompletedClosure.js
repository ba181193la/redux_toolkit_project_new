...
const FilterFormCompletedClosure = ({ fields, handleToggleFilter, labels }) => {
  ...
  const fieldsConfig = [
    ...
    },
    {
      ...
      component: 'Dropdown',
      name: 'incidentType',
      options: pageLoadData?.Data.IncidentTypeList?.map(
        (data) => ({
    ...
    },
    {
      ...
      component: 'Dropdown',
      name: 'department',
      options: pageLoadData?.Data.DepartmentList?.filter((data) => data?.DepartmentName && data?.DepartmentId)
      .map((data) => ({
    ...
    },
    {
      ...
      component: 'Dropdown',
      name: 'mainCategory',
      options: pageLoadData?.Data.IncidentCategoryList?.map(
        (data) => ({
    ...
    <>
      ...
      <Formik
        ...
        onSubmit={async (values) => {
          const formattedFacilityIds = Array.isArray(values.facility) ? values.facility.join(',') : '';
          ...
          const filters = {
            ...
            incidentLevelId: values.incidentLevel  || 0, 
...
export default FilterFormCompletedClosure;
