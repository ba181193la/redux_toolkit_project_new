...
import { useSkipInvestigationMutation, useFillRCAMutation,useGetReportIncidentPageloadQuery,useGetApprovalEntryDataQuery,useAssignInvestigatorMutation,useOpinionMutation } from '../../../../redux/RTK/IncidentManagement/incidentApprovalApi'
...
const PendingApproval = () => {
  ...
  // Accordians 
    ...
    const { data: entrydetails = [], isFetching: isEntrydetails } = useGetApprovalEntryDataQuery({
      incidentId : id,
    ...
    const [additionalselectedValue, setAdditionalSelectedValue] = useState(entrydetails?.Data?.AdditionalStaffNotify || "No");
    const [selectedClinicalValue, setSelectedClinicalValue] = useState(entrydetails?.Data?.ClinicalType || 'Clinical');
    const [incidentMainCategory, setIncidentMainCategory] = useState({ id: "", name: "" });
    const [incidentSubCategory, setIncidentSubCategory] = useState({ id: "", name: "" });
    const [incidentDetails, setIncidentDetails] = useState({ id: "", name: "" });
    const [incidentType, setIncidentType] = useState({ id: "", name: "" });
    const [incidentDepartment, setIncidentDepartment] = useState({ id: "", name: "" });
  ...
  const handleSelectRow = (incident) => {
    ...
    setIncidentMainCategory({id:incident.MainCategoryId,name: incident.MainCategory});
    setIncidentSubCategory({id:incident.SubCategoryId,name: incident.SubCategory});
    setIncidentDetails({id:incident.IncidentDetailId,name: incident.IncidentDetail});
...
// Page load Data 
  ...
  const { data: pageloadData = [], isFetching: ispageloadData } = useGetReportIncidentPageloadQuery({
...
useEffect(() => {
  if (entrydetails?.Data) {
    ...
    setIncidentMainCategory({id:entrydetails.Data.MainCategoryId,name: entrydetails.Data.MainCategory});
    setIncidentSubCategory({id:entrydetails.Data.SubCategoryId,name: entrydetails.Data.SubCategory});
    setIncidentDetails({id:entrydetails.Data.IncidentDetailId,name: entrydetails.Data.IncidentDetail});
    
    setIncidentType({id:entrydetails.Data.IncidentTypeId,name: entrydetails.Data.IncidentType})
    setIncidentDepartment({id: entrydetails.Data.DepartmentId,name: entrydetails.Data.DepartmentName});
    setHarmLevel({id: entrydetails.Data.IncidentHarmLevelId,name: entrydetails.Data.IncidentHarmLevel});
...
const common_data = {
  ...
  const htmlToText = (html) => {
    ...
    const processedText = extractText(tempDiv)
      .replace(/\n{3,}/g, '\n\n')
      .replace(/[ \t]+/g, ' ')
      .trim();
   ...
   const handlePrint = () => {
        try {
          ...
          const pdfFileName = 'Incident_closure_pending.pdf';
          const margin = 40;
          ...
          console.log(reportIncidentSafe, 'datas.. in pdfff');
          ...
          if (introDataContainer) {
            const introText = htmlToText(introData);
            ...
            if (introText) {
              ...
              const lineHeight = 15;
              const pageBottom = pageHeight - margin - 50;
          
              let startY = doc.lastAutoTable 
                ? doc.lastAutoTable.finalY + 40  
                : margin + 100;
          
              if (startY + lineHeight * 3 > pageBottom) {
                doc.addPage();
                addHeaderFooter(); 
                startY = margin + 100;
              }
              ...
              introLines.forEach((line) => {
                if (line.trim().toUpperCase() === line.trim()) {
                  ...
                  doc.setFontSize(12);
                  doc.setFont('helvetica', 'normal');
                  doc.text(line, leftMargin + 10, startY);
                } else {
                  doc.setFontSize(12);
                  doc.setFont('helvetica', 'normal');
                  doc.text(line, leftMargin, startY);
                }
          
                if (startY + lineHeight > pageBottom) {
                  doc.addPage();
                  addHeaderFooter(); 
                  startY = margin + 100;
                  doc.setFont('helvetica', 'normal');
          ...
          if (actionContainer) {
            const introText = htmlToText(ActionPlanData);
    ...
    <FlexContainer style={{ flexDirection: 'column', width: '100%' }}>
      ...
      <FlexContainer style={{ backgroundColor: '#fff' }}>
        <Box p={3} paddingTop={0} style={{ width: '-webkit-fill-available' }}>
            ...
            <FormContainer style={{ marginBottom: '20px' }}>
              <FlexContainer flexDirection={'column'}>
                ...
                <Grid container spacing={2} p={2} style={{ padding: '0px' }}>
                  ...
                  <FormLabel sx={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {['IncidentMainCategory', 'IncidentSubCategory', 'IncidentDetails'].map((name) => {
                      ...
                      if(field) {
                        return (
                          <Grid item xs={4} key={name}>
                            ...
                            <TextField
                              fullWidth
                              value={name === 'IncidentMainCategory' ? incidentMainCategory.name :
                                    name === 'IncidentSubCategory' ? incidentSubCategory.name :
                                    name === 'IncidentDetails' && incidentDetails.name ? incidentDetails.name : incidentSubCategory.name} // Show SubCategory if IncidentDetails is empty or null
                              slotProps={{ htmlInput: { maxLength: 200, style: { color: 'black', fontWeight: 'bold', fontSize: '15px' } } }}
                              disabled
                            />
                          </Grid>
                        );
                    ...
                    <Grid item xs={4}>
                      ...
                      <Dropdown
                        options={pageloadData?.Data?.IncidentTypeList?.map((item) => ({
                 ...
                 {getlabel(filteredIncidentApprovalConfig.find((config) => config.name === 'Clinical/NonClinical')?.translationId, filterIncidentApprovalLabels, i18n.language)}
                  ...
                  {getlabel(filteredIncidentApprovalConfig.find((config) => config.name === 'IncidentDepartment')?.translationId, filterIncidentApprovalLabels, i18n.language)}
                    ...
                    <Dropdown
                          options={pageloadData?.Data?.DepartmentList?.map((item) => ({
                  ...
                  {getlabel(filteredIncidentApprovalConfig.find((config) => config.name === 'HarmLevel')?.translationId, filterIncidentApprovalLabels, i18n.language)}
                      ...
                      <Dropdown
                          options={pageloadData?.Data?.HarmLevelList?.map((item) => ({
          ...
          <Grid item xs={12}>
          {selectedAction === 'merge' && <MergeIncident entryData = {entrydetails?.Data} common_data = {common_data} />}
          {selectedAction === 'request' && <RequestInformation entryData = {entrydetails?.Data} />}
          {selectedAction === 'reject' && <RejectIncident entryData = {entrydetails?.Data} />}
          ...
          {/* Add other conditions for other actions */}
              ...
              <Dialog open={openSkillPopup} onClose={handleSkilCancel}   PaperProps={{
                style: {
                  width: '360px', 
                },
              }}>
                    ...
                    <h3 style={{color: '#575757'}}><strong>Confirm to Submit?</strong></h3>
                    Incident will be Approved and Redirected to the Incident Closure Page
                    </DialogContent >
            ...
            <Dialog open={openRCAPopup} onClose={handleRCACancel}   PaperProps={{
                style: {
                  width: '360px', 
                    ...
                    <DialogContent  sx={{ textAlign: 'center', padding: 0 }}>
                    <h3 style={{color: '#575757'}}><strong>Confirm to Submit?</strong></h3>
                    Incident will be Approved and Redirected to the Root Cause Analysis Page
                    </DialogContent >
                    <DialogActions sx={{ justifyContent: 'center', gap: 2, marginBottom: '12px'}}>
                       <ActionButton
                                    variant="outlined"
