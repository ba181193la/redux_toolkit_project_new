...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'Questions')?.isShow && (
        <StyledTableBodyCell>{item?.Questions}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};

const DraggableTable = ({ tableColumnList, fieldLabels, labelsStatus }) => {
  //* Selectors
  ...
  useEffect(() => {
    if (QuestionsDetails?.Records) {
      setTableData(QuestionsDetails.Records);
    }
  }, [QuestionsDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      rcaQuestionstId: item.RCAQuestionstId,
...
const ReorderTable = ({ tableColumnList, fieldLabels, labelsStatus }) => {
  //* Hooks declaration
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
