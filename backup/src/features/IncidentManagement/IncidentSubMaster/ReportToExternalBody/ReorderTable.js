...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'ExternalBody')?.isShow && (
        <StyledTableBodyCell>{item?.ExternalBody}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (externlBodyDetails?.Records) {
      setTableData(externlBodyDetails?.Records);
    }
  }, [externlBodyDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      externalBodyId: item.ExternalBodyId,
