...
import AddEditIncidentType from './AddEditIncidentType';
import {
...
import SentinelEventDataTable from './SentinelEventDataTable';

const IncidentType = ({ labels, fieldAccess, expandAll }) => {
  //* Hooks declaration
  ...
  const { incidentTypePageIndex: pageIndex, incidentTypePageSize: pageSize } =
  ...
  const handleOnPageChange = (event, newPage) => {
    dispatch(setIncidentTypePageIndex(newPage));
  };

  const handleOnPageSizeChange = (event) => {
    dispatch(setIncidentTypePageSize(parseInt(event.target.value, 10)));
    dispatch(setIncidentTypePageIndex(0));
  };
  ...
  const tableColumnList = [
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'IS_IT_P_Facility')?.IsShow,
    },
    {
      id: 'IncidentTypeName',
      translationId: 'IM_IS_IT_IncidentType',
      isShow: pageFields?.find((x) => x.FieldId === 'IS_IT_P_IncidentType')
        ?.IsShow,
    },
    {
      id: 'TypeDefinition',
      translationId: 'IM_IS_IT_Definition',
      isShow: pageFields?.find((x) => x.FieldId === 'IS_IT_P_Definition')
        ?.IsShow,
    },
    {
      id: 'IsSentinel',
      translationId: 'IM_IS_IT_IsSentinel',
    ...
    {
      id: 'NotifyStaffs',
      translationId: 'IM_IS_IT_NotificationStaff(s)',
  ...
  //* RTK Queries
  const [triggerDeleteReferenceNumber] = useDeleteIncidentTypeMutation();
  const [triggerDownloadData] = useDownloadIncidentTypeMutation();
  ...
  useEffect(() => {
    ...
    if (incidentTypeDetails?.Records) {
      setIncidentTypeList(incidentTypeDetails?.Records);
      setTotalRecords(incidentTypeDetails?.TotalRecords);
    }
  }, [pageLoadData, incidentTypeDetails]);
  ...
  //* Edit incident type details
  const editIncidentType = (incTypeId) => {
    setSelectedIncidentTypeId(incTypeId);
    setShowIncidentTypeModal(true);
  };

  //* Delete incident type
  const deleteIncidentType = (incTypeId) => {
    const callback = async () => {
      try {
        await triggerDeleteReferenceNumber({
          IncidentTypeId: incTypeId,
          loginUserId: userDetails?.UserId,
          menuId: selectedMenu?.id,
        }).unwrap();
        ...
        showSweetAlert({
          type: 'success',
          title: 'Deleted!',
          text: 'Incident Type has been deleted.',
          timer: 3000,
          gif: SuccessGif,
          height: 250,
          width: 250,
  ...
  return (
    <>
                ...
                {checkAccess(
                  ...
                  <Tooltip title="Reorder" arrow>
                    <StyledImage
                      ...
                      animate={true}
                      onClick={() => {
                        navigate('/IncidentManagement/ReorderIncidentType');
                      }}
                    />
                  </Tooltip>
                )}
      ...
      {showIncidentTypeModal && (
        <AddEditIncidentType
          showIncidentTypeModal={showIncidentTypeModal}
          setShowIncidentTypeModal={setShowIncidentTypeModal}
          selectedIncidentTypeId={selectedIncidentTypeId}
          setSelectedIncidentTypeId={setSelectedIncidentTypeId}
