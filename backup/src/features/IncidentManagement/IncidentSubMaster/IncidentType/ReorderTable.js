...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'IncidentTypeName')?.isShow && (
        <StyledTableBodyCell>{item?.IncidentTypeName}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'TypeDefinition')?.isShow && (
        <StyledTableBodyCell>{item?.TypeDefinition}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'IsSentinel')?.isShow && (
        <StyledTableBodyCell>{item?.IsSentinel}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (incidentTypeDetails?.Records) {
      setTableData(incidentTypeDetails?.Records);
    }
  }, [incidentTypeDetails]);
