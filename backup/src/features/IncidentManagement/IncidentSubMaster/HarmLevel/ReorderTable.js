...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'IncidentHarmLevel')?.isShow && (
        <StyledTableBodyCell>{item?.IncidentHarmLevel}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'HarmLevelDefinition')?.isShow && (
        <StyledTableBodyCell>{item?.HarmLevelDefinition}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (harmLevelDetails?.Records) {
      setTableData(harmLevelDetails?.Records);
    }
  }, [harmLevelDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      incidentHarmLevelId: item.IncidentHarmLevelId,
