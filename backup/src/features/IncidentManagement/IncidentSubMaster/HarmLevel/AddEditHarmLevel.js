...
const AddEditHarmLevel = ({
  ...
  useEffect(() => {
    if (harmLevelRecords?.Data && selectedHarmLevelId) {
      setNewHarmLevel({
        ...
        harmLevelCode: harmLevelRecords?.Data?.IncidentHarmLevelCode,
  ...
  const harmLevelValidation = Yup.object().shape({
    ...
    harmLevelCode: Yup.string().when([], {
      is: () =>
        pageFields?.find((x) => x.FieldId === 'IS_HL_P_HarmLevelCode')
          ?.IsMandatory === true && regionCode.toUpperCase() === 'ABD',
      then: (schema) =>
        schema.required(
          `${getlabel(
            'IM_IS_HL_HarmLevelCode',
            {
              Data: fieldLabels,
              Status: labels?.Status,
            },
      ...
      if (newHarmLevel?.harmLevelId === 0) {
        response = await triggerAddHarmLevel({
          payload: {
            ...
            harmLevelCode: values?.harmLevelCode,
              ...
              {({ values, errors, setFieldValue, touched }) => (
                ...
                >
                  ...
                  >
                    ...
                    {pageFields?.find(
                      (x) => x.FieldId === 'IS_HL_P_HarmLevelCode'
                    )?.IsShow &&
                      regionCode.toUpperCase() === 'ABD' && (
                        ...
                        >
                          ...
                          <Grid item xs={12} sm={12} md={8} lg={8}>
                            <Field name="harmLevelCode">
                              {({ field }) => (
                                <TextField
                                  {...field}
                                  id="harmLevelCode"
                                  autoComplete="off"
                                  values={values.harmLevelCode}
                                  fullWidth={true}
                                  slotProps={{
