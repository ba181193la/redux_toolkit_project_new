...
const AttachmentTable = ({ labels, data }) => {
  ...
  const { data: attachmentData, isSuccess } = useGetAttachmentDownloadQuery(
   ...
   const getAttachmentLink = (row) => {
    const autogenFileURL = row.AutogenFilePath.replace(/\\/g, '/');

    const originalFileName = row?.OriginalFileName;
    
    if (!autogenFileURL || !originalFileName) {
      console.error("Missing required attachment data");
      return;
    }
    
    setDownloadParams({
      AutogenFileURL: autogenFileURL,
