...
const RootCauseTable = ({ columns, labels, data }) => {
  ...
  console.log('rootdatall', rootTableData);
  ...
  useEffect(() => {
    if (rootTableData && rootTableData.length > 0) {
      setTableData(parseInitialRows(rootTableData));
    }
  }, [rootTableData]);
  ...
  const handleSave = (r) => {
    if (r.AssignedTask && r.TargetDate && r.UserName) {
      ...
      console.log('updatedRowshhhhh', updatedRows);
    } else {
                      ...
                      <StyledTableBodyCell>
                        {row.isEditable ? (
                          <TextField
                            ...
                            disabled={row.isManuallyAdded === false}
                            onChange={(e) =>
                              handleInputChange(
                                e,
                                row.RowNo,
                    ...
                    <FlexContainer className="action-icons">
                      ...
                      <Tooltip title="Delete" arrow>
                        <StyledImage
                          ...
                          alt="Delete"
                          onClick={() => {
                            if (row.isManuallyAdded !== false) {
                              handleDeleteClick(row);
                            }
                          }}
                          style={{
                            opacity: row.isManuallyAdded === false ? 0.5 : 1,
                            pointerEvents:
                              row.isManuallyAdded === false ? 'none' : 'auto',
                          }}
                        />
                      </Tooltip>
                    </FlexContainer>
