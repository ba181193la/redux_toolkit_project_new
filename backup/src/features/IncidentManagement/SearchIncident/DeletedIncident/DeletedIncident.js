...
import DeletedIncidentTable from './DeletedIncidentTable';
import DeletedData from './DeletedIncidentTable.json';
import FilterFormDeleted from './FilterFormDeleted';
...
const StyledFlexContainer = styled(FlexContainer)`
  ...
  ${media.screen.small`
    ...
    `};
`;
const DeletedIncident = (IsDetailView) => {
  //*Hooks declaration

  const [seletedDeletedColumns, setSelectedDeletedColumns] =
    useState(DeletedData);
  ...
  const [triggerDownloadData] = useGetDownloadDataDeleteMutation();
  ...
  const handleOnDownloadOrPrint = async (fileType) => {
    if (filteredRecords?.length > 0) {
      try {
        ...
        if (fileType === 'Pdf') {
          saveAs(blob, 'Delete Incident.pdf');
          setMessage('Download completed');
          setTimeout(() => {}, 2000);
        } else {
          saveAs(blob, 'Delete Incident.xlsx');
          setMessage('Download completed');
          setTimeout(() => {}, 2000);
        }
      } catch (error) {
  ...
  const { totalRecords, records } = getDeletedIncident || {
    totalRecords: 0,
    records: [],
  };
  ...
  useEffect(() => {
    if (incidentLabels) {
      const pageFields = incidentLabels?.Sections?.find(
...
export default DeletedIncident;
