...
import SubmittedByMeTable from './SubmittedByMeTable';
import SubmittedData from './SubmittedByMe.json';
import FilterFormSubmitted from './FilterFormSubmitted';
...
const SubmittedByMe = (IsDetailView) => {
  const [seletedSubmittedColumns, setSelectedSubmittedColumns] =
  ...
  const handleOnDownloadOrPrint = async (fileType) => {
    if (filteredRecords?.length > 0) {
      try {
        ...
        if (fileType === 'Pdf') {
          saveAs(blob, 'Submitted By.pdf');
          setMessage('Download completed');
          setTimeout(() => {}, 2000);
        } else {
          saveAs(blob, 'Submitted By.xlsx');
          setMessage('Download completed');
          setTimeout(() => {}, 2000);
        }
      } catch (error) {
  ...
  const submittedFields = menuData.find(
    (menu) => menu.MenuId === selectedMenu?.id
  );

  const submittedLabels = LabelData.find(
    (menu) => menu.MenuId === selectedMenu?.id
  );

  const alteredLabels = [
    ...(submittedLabels?.Regions?.find((item) => item.RegionCode === 'All')
      ?.Labels || []),
  ];
  ...
  const { totalRecords, records } = getIncidentSubmitted || {
    totalRecords: 0,
    records: [],
  };
  ...
  useEffect(() => {
    if (submittedFields) {
      const pageFields = submittedFields?.Sections?.find(
