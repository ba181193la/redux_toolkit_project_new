...
import { setSelectedFacility } from '../../../../redux/features/auth/authSlice';

const MainCategory = ({ labels, fieldAccess }) => {
  //* Hooks declaration
  ...
  const { mainCategoryPageIndex: pageIndex, mainCategoryPageSize: pageSize } =
    useSelector((state) => state.IncidentCategory.mainCategory);
  ...
  const handleOnPageChange = (event, newPage) => {
    dispatch(setMainCategoryPageIndex(newPage));
  };

  const handleOnPageSizeChange = (event) => {
    dispatch(setMainCategoryPageSize(parseInt(event.target.value, 10)));
    dispatch(setMainCategoryPageIndex(0));
  };
  ...
  const orderedMainCategoryList = [...mainCategoryList].sort((a, b) => {
  ...
  const [triggerDownloadData] = useDownloadMainCategoryMutation();
  const [triggerDeleteMainCategory] = useDeleteMainCategoryMutation();

  const {
    data: mainCategoryDetails,
    isFetching,
    refetch,
  } = useGetAllMainCategoryQuery(
    {
  ...
  useEffect(() => {
    if (mainCategoryDetails?.Records) {
      setMainCategoryList(mainCategoryDetails?.Records);
      setTotalRecords(mainCategoryDetails?.TotalRecords);
    }
  }, [mainCategoryDetails]);
  ...
  const tableColumnList = [
    {
      id: 'Facility',
      translationId: 'IM_IC_MC_Facility',
      isShow: pageFields?.find((x) => x.FieldId === 'IC_MC_P_Facility')?.IsShow,
    },
    {
      id: 'AffectedCategoryCode',
      translationId: 'IM_IC_MC_AffectedCategoryCode',
      isShow:
    ...
    {
      id: 'AffectedCategory',
      translationId: 'IM_IC_MC_AffectedCategory',
      isShow:
        pageFields?.find((x) => x.FieldId === 'IC_MC_P_AffectedCategoryCode')
          ?.IsShow && !hideABDFields,
    },
    {
      id: 'MainCategoryCode',
      translationId: 'IM_IC_MC_MainCategoryCode',
      isShow:
    ...
    {
      id: 'MainCategory',
      translationId: 'IM_IC_MC_MainCategory',
      isShow: pageFields?.find((x) => x.FieldId === 'IC_MC_P_MainCategory')
        ?.IsShow,
    },
    {
      id: 'NotifyStaffsName',
      translationId: 'IM_IC_MC_NotificationStaff(s)',
    ...
    {
      id: 'AddedEditedBy',
      translationId: 'IM_IC_MC_AddedEditedBy',
      isShow: pageFields?.find((x) => x.FieldId === 'IC_MC_P_AddedEditedBy')
        ?.IsShow,
    },
    {
      id: 'AddedEditedDate',
      translationId: 'IM_IC_MC_AddedEditedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'IC_MC_P_AddedEditedDate')
        ?.IsShow,
    },
  ];
  ...
  const deleteMainCategory = (mainCatId) => {
    const callback = async () => {
      try {
        await triggerDeleteMainCategory({
            ...
            >
              ...
              >
                {checkAccess(
                  ...
                  <StyledButton
                    onClick={() => {
                      navigate('/IncidentManagement/MainCategoryQuestions');
                    }}
                    disabled={isFetching}
                    fontSize="13px"
                    backgroundColor="#0083c0"
              ...
              >
                {isFetching ? (
                  ...
                  <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
                    ...
                    <TableBody>
                      {orderedMainCategoryList?.length > 0 ? (
                        orderedMainCategoryList?.map((row, rowIndex) => (
                                  ...
                                  {checkAccess(
                                    ...
                                    <Tooltip title="Edit" arrow>
                                      <StyledImage
                                        ...
                                        alt="Edit"
                                        onClick={() =>
                                          editMainCategory(row?.MainCategoryId)
                                        }
                                      />
                                    </Tooltip>
                                  )}
