...
const SubCategoryQuestions = () => {
  ...
  const [triggerGetFormBuilderData] = useLazyGetSCFormBuilderByIdQuery();

  const [triggerSaveSCFormBuilder, { isLoading: isAdding }] =
    useSaveSCFormBuilderMutation();
  ...
  const subCategoryValidation = Yup.object().shape({
    ...
    affectedCategory: Yup.string().when([], {
      is: () =>
        (pageFields?.find((x) => x.FieldId === 'IC_SC_P_AffectedCategory')
          ?.IsShow ||
    ...
    mainCategory: Yup.string().when([], {
      is: () =>
        pageFields?.find((x) => x.FieldId === 'IC_SC_P_MainCategory')?.IsShow ||
    ...
    subCategory: Yup.string().when([], {
      is: () =>
        pageFields?.find((x) => x.FieldId === 'IC_SC_P_SubCategory')?.IsShow ||
    ...
    pageName: Yup.string().when([], {
      is: () =>
        pageFields?.find((x) => x.FieldId === 'IC_SC_P_PageName')?.IsShow,
      then: (schema) =>
        schema.required(
          `${getlabel(
            'IM_IC_SC_PageName',
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      response = await triggerSaveSCFormBuilder({
        payload: {
          pageMenuId: selectedMenu?.id,
          pageModuleId: selectedModuleId,
          loginUserId: userDetails?.UserId,
          ...
          subCategoryId: initalValues?.subCategory,
          formData: data,
        },
      });
      if (
              ...
              {({ values, errors, setFieldValue, touched }) => (
                <Form style={{ width: '100%' }}>
                  <Grid spacing={2} container alignItems={'flex-start'}>
                      ...
                      <Grid item xs={12} sm={12} md={6} lg={3}>
                        <Label
                          ...
                          isRequired={
                            pageFields?.find(
                              (x) =>
                                x.FieldId === 'IC_SC_P_AffectedCategoryCode'
                            )?.IsShow ||
                            pageFields?.find(
                              (x) => x.FieldId === 'IC_SC_P_AffectedCategory'
                            )?.IsShow
                    ...
                    {pageFields?.find((x) => x.FieldId === 'IC_SC_P_PageName')
                      ?.IsShow && (
                      <Grid item xs={12} sm={12} md={6} lg={3}>
                        <Label
                          value={getlabel(
