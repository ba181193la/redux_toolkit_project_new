...
const AffectedCategory = ({
  ...
  useEffect(() => {
      ...
      if (selectedCategory || searchQuery) {
        try {
          const response = await getIncidentCategoryList({
            affectedCategoryId: selectedCategory,
            keyword: searchQuery,
    ...
    fetchIncidentList();
  }, [selectedCategory, searchQuery, getIncidentCategoryList, pageSize]);
                ...
                {incidentListitems?.length > 0 ? (
                  incidentListitems.map((incident, index) => (
                    ...
                    >
                      ...
                      <StyledTableBodyCell>
                        {incident.MainCategoryCode} - {incident.MainCategory}
                      </StyledTableBodyCell>
                      <StyledTableBodyCell>
                        {incident.IncidentDetailCode} -{' '}
