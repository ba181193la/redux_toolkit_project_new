...
import FilterFormCompletedClosure from './FilterFormCompletedClosure';
import styled from 'styled-components';
import FilterCloseIcon from '../../../../assets/Icons/FilterCloseIcon.png';
import DataTableComplete from './DataTableComplete';
...
const ClosureCompleted = () => {
  ...
  const [triggerDownloadData] = useGetDownloadDataClosureCompleteMutation();
  ...
  const handleOnDownloadOrPrint = async (fileType) => {
    if (filteredRecords?.length > 0) {
      try {
        ...
        if (fileType === 'Pdf') {
          saveAs(blob, 'Incident Closure Completed.pdf');
          setMessage('Download completed');
          setTimeout(() => {
            setIsLoading(false);
          }, 2000);
        } else {
          saveAs(blob, 'Incident Closure Completed.xlsx');
          setMessage('Download completed');
          setTimeout(() => {
            setIsLoading(false);
          }, 2000);
  ...
  const {
    data: getIncidentClosureCompleted = [],
    isFetching,
    refetch,
  } = useGetIncidentClosureCompletedQuery(
    {
  ...
  const { totalRecords, records } = getIncidentClosureCompleted || {
    totalRecords: 0,
    records: [],
  };
  ...
  const labels = (labelsData.Data || [])
    ...
    .flatMap((item) =>
      (item.Regions || [])
    .filter((region) => region.RegionCode === 'ALL' || region.RegionCode === 'ABD')
  ...
  const fields =
    fieldsData?.Data?.Menus?.filter((item) => item.MenuId === 30) || [];
...
export default ClosureCompleted;
