...
const ClosureDrafted = () => {
  ...
  const handleOnDownloadOrPrint = async (fileType) => {
    if (filteredRecords?.length > 0) {
      ...
      try {
        ...
        if (fileType === 'Pdf') {
          console.log('success');
          saveAs(blob, 'Incident Closure Drafted.pdf');
          ...
          saveAs(blob, 'Incident Closure Drafted.xlsx');
          setMessage('Download completed');
          setTimeout(() => {
            setIsLoading(false);
          }, 2000);
  ...
  const {
    data: getIncidentClosureDrafted = [],
    isFetching,
    refetch,
  } = useGetIncidentClosureDraftedQuery(
    {
  ...
  const { totalRecords, records } = getIncidentClosureDrafted || {
    totalRecords: 0,
    records: [],
  };
