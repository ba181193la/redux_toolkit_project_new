...
const IncidentLevel = ({
  ...
  const handleRCASubmit = async (values) => {
    ...
    try {
      ...
      formData.append('fillRCAClosureSave', JSON.stringify(result));

      const response = await triggerFillRCA({ formData });
  ...
  const handleSkipRca = async (values) => {
    ...
    try {
      ...
      //        },
      //      );
        //  const filterData = data?.incidentRiskLevelClosure;
      //    console.log("getIncidentLevelData",getIncidentLevelData)
  ...
  return (
    <FlexContainer flexDirection="column" width="100%">
      ...
      <Dialog open={openSecondPopup} onClose={handleSecondPopupCancel} PaperProps={{ style: { width: '360px' } }}>
        ...
        <DialogContent sx={{ textAlign: 'center', padding: 0 }}>
          {isRCAActionSelected === 'fill' ? (
            <h3 style={{ color: '#575757' }}>
              <strong>Confirm to Draft Incident Closure & Fill RCA</strong>
            </h3>
          ) : isRCAActionSelected === 'skip' ? (
            <p style={{ color: '#575757' }}>
              Skipping RCA will delete any drafted RCA for the Incident
        ...
        <DialogActions
          sx={{ justifyContent: 'center', gap: 2, marginBottom: '12px' }}
        >
          <ActionButton
            variant="outlined"
            sx={{
            ...
            startIcon={
              <StyledImage
                ...
                style={{ marginBottom: '1px', marginInlineEnd: 8 }}
              />
            }
            onClick={handleRCACancel}
          >
