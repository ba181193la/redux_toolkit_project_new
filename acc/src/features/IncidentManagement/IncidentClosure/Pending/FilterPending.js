...
const ClosurePending = () => {
  ...
  const [triggerDownloadData] = useGetDownloadDataClosurePendingMutation();
  ...
  const handleOnDownloadOrPrint = async (fileType) => {
    if (filteredRecords?.length > 0) {
      try {
        ...
        if (fileType === 'Pdf') {
          console.log('success');
          saveAs(blob, 'Incident Closure Pending.pdf');
          setMessage('Download completed');
          setTimeout(() => {
            setIsLoading(false);
          }, 2000);
        } else {
          saveAs(blob, 'Incident Closure Pending.xlsx');
          setMessage('Download completed');
          setTimeout(() => {
            setIsLoading(false);
          }, 2000);
  ...
  const { totalRecords, records } = getIncidentClosurePending || {
    totalRecords: 0,
    records: [],
  };
