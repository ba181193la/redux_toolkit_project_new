...
const QuestionAnswers = () => {
  ...
  const handleRadioChange = (event, questionId) => {
    ...
    const updatedQuestions = QuestionAnswersData.map((q) =>
      q.RCAQuestionstId === questionId ? { ...q, RootCause: newRootCause } : q
    );
    ...
    if (newRootCause !== 'Yes') {
      const filteredRootTableData = existingRootTableData.filter(
        (entry) => entry.RCAQuestionstId !== questionId
      );
...
const updatedRootTableData = existingRootTableData.some(entry => entry.RCAQuestionstId === questionId)
  ? existingRootTableData.map(entry =>
      entry.RCAQuestionstId === questionId
  ...
  const handleTextChange = (event, questionId) => {
    ...
    const updatedQuestions = QuestionAnswersData.map((q) =>
      q.RCAQuestionstId === questionId ? { ...q, RootCauseFinding: newValue } : q
    );

    dispatch(setQuestionAnswersData(updatedQuestions));
  };

  const handleNotApplicableChange = (event, questionId) => {
    const newValue = event.target.checked;
  
    // setNotApplicable(newValue);
  
    const updatedQuestions = QuestionAnswersData.map((q) =>
      q.RCAQuestionstId === questionId
        ? {
            ...q,
    ...
    const updatedRootTableData = newValue
      ? RootTableData.filter(entry => entry.RCAQuestionstId !== questionId)
      : RootTableData;
...
<Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
  ...
  <Box sx={{ display: 'flex', alignItems: 'center', ml: 'auto' }}>
    <Checkbox
      checked={currentQuestionData?.IsNotApplicable || false} 
      onChange={(e) => handleNotApplicableChange(e, currentQuestionData?.RCAQuestionstId)} 
    ...
    <Typography component="span" sx={{ mr: 1, color: '#333' }}>
      Not Applicable
            ...
            <TextField
              ...
              onChange={(event) => {
                handleTextChange(event, currentQuestionData?.RCAQuestionstId);
                console.log(
                  'currentQuestionData?.RCAQuestionstId',
                  currentQuestionData?.RCAQuestionstId
                );
              }}
              sx={{
