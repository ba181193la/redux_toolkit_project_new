...
export default function BasicTabs() {
 ...
 const rcaQuestions = entryData?.Data?.rcaQuestions;
  ...
  const FishboneAttachments = [
    {
      FishboneOriginalFileName: entryData?.Data?.FishboneOriginalFileName,
      FishboneAutogenFilePath: entryData?.Data?.FishboneAutogenFilePath,
    },
  ];
  ...
  console.log("mappedEventData",mappedEventData)

  const mappedTeamData = teamMembersRCA?.map((team, index) => ({
  ...
  const mappedRootData = rcaActions?.map((root, index) => ({
    Department: root.Department || '',
    UserName: root.UserName || '',
    TargetDate: root.TargetDate || '',
    AssignedTask: root.AssignedTask || '',
    RootCauseFinding: root.RootCauseFinding || '',
    RootCauseReference: root.RootCauseReference || '',
    UserId: root.UserId,
    isEditable: false,
    RowNo: index + 1,
  ...
  const mappedMailToData = mailToStaffs?.map((mail, index) => ({
  ...
  useEffect(() => {
    dispatch(setTeamTableData(mappedTeamData));
    dispatch(setRootTableData(mappedRootData));
    dispatch(setMailToTableData(mappedMailToData));
    dispatch(setEventTableData(mappedEventData));
    dispatch(setQuestionAnswersData(rcaQuestions));
    dispatch(setFishBoneAttachmentData(FishBoneData));
    dispatch(setAttachmentData(formattedAttachments));
  }, [entryData, FishBoneData]);
