...
import { useGetCategoryDetailsQuery, useGetConsequencelevelDetailsQuery, useGetLikelihoodDetailsQuery, useReorderCategoryAffectedMutation, useReorderConsequenceLevelMutation, useReorderliklihoodMutation, useUpdateConsequenceLevelMutation } from '../../../../redux/RTK/incidentRiskLevelApi';
...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'Likelihood') && (
        <StyledTableBodyCell>{item?.Likelihood}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'LikelihoodDefinition') && (
        <StyledTableBodyCell>{item?.LikelihoodDefinition}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'Score') && (
        <StyledTableBodyCell>{item?.Score}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const DraggableTable = ({
 ...
 const {
  ...
  useEffect(() => {
    if (getLikelihoodDetails?.Records) {
      setTableData(getLikelihoodDetails?.Records);
    }
  }, [getLikelihoodDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      ...
      likelihoodDefinition: item.LikelihoodDefinition,
