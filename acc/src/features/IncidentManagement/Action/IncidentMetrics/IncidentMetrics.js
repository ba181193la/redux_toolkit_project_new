...
const ActionMetrics = () => {
    ...
    const { Pending, Completed, ApprovalPending, LastWeekRecord } = pageLoadData?.Data?.Result || {};
  ...
  return (
    ...
    >
      <StyledFlexContainer
        ...
        {userDetails?.StaffCategoryName === 'Incident Module Admin' && (
<StyledFlexContainer
  style={{
    display: 'flex',
...
>
  ...
  >
    <div style={{ fontSize: '18px', }}>{ApprovalPending}</div>
    <div style={{ fontWeight: 'bold', fontSize: '16px' }}>ApprovalPending</div>
  </div>
</StyledFlexContainer>
        )}
