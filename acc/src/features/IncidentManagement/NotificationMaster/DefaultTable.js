...
const DefaultTable = ({ columns = [], labels, isView = false }) => {
  ...
  const validationSchema = Yup.object({
    ...
    yesNo: Yup.string().required('Please select To or CC'),
  });
      ...
      >
        <Formik
          ...
          onSubmit={(values, { resetForm }) => {
            ...
            const isRoleExisting = roleList.some((role) => role.RoleId === values?.role);
            if (!isRoleExisting) {
              dispatch(setRoleList([...roleList, updatedRole]));
            } else {
              console.log("RoleId already exists in the roleList. No update made.");
            }
            resetForm();
          }}
        >
