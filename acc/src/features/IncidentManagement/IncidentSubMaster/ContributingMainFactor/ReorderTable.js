...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'ContributingFactorCode')
        ?.isShow && (
        <StyledTableBodyCell>
          {item?.ContributingFactorCode}
        </StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'ContributingFactor')?.isShow && (
        <StyledTableBodyCell>{item?.ContributingFactor}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (mainFactorDetails?.Records) {
      setTableData(mainFactorDetails?.Records);
    }
  }, [mainFactorDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList?.map((item, index) => ({
      mainFactorId: item.MainFactorId,
