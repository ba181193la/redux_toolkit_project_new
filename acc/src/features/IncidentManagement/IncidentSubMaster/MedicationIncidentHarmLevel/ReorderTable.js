...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'IncidentMedicationHarmLevel')
        ?.isShow && (
        <StyledTableBodyCell>
          {item?.IncidentMedicationHarmLevel}
        </StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'MedicationHarmDefinition')
        ?.isShow && (
        <StyledTableBodyCell>
          {item?.MedicationHarmDefinition}
        </StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (medicationHarmLevelDetails?.Records) {
      setTableData(medicationHarmLevelDetails?.Records);
    }
  }, [medicationHarmLevelDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      incidentMedicationHarmId: item.IncidentMedicationHarmId,
