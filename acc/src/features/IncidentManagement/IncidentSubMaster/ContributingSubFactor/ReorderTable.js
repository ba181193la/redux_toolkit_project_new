...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'MainFactorCode')?.isShow && (
        <StyledTableBodyCell>{item?.MainFactorCode}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'MainFactor')?.isShow && (
        <StyledTableBodyCell>{item?.MainFactor}</StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'ContributingSubFactorCode')
        ?.isShow && (
        <StyledTableBodyCell>
          {item?.ContributingSubFactorCode}
        </StyledTableBodyCell>
      )}
      {tableColumnList?.find((x) => x.id === 'ContributingSubFactor')
        ?.isShow && (
        <StyledTableBodyCell>{item?.ContributingSubFactor}</StyledTableBodyCell>
      )}
    </StyledTableRow>
  );
};
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (subFactorDetails?.Records) {
      setTableData(subFactorDetails?.Records);
    }
  }, [subFactorDetails]);
  ...
  handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Failed to reorder Main Factor', e);
    }
  };
