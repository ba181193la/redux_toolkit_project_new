...
const DraggableRow = ({ item, index, moveRow, tableColumnList }) => {
  ...
  return (
    ...
    >
      ...
      {tableColumnList?.find((x) => x.id === 'JAWDALevel')?.isShow && (
        <StyledTableBodyCell>{item?.JAWDALevel}</StyledTableBodyCell>
      )}
...
const DraggableTable = ({
  ...
  useEffect(() => {
    if (jawdaLevelDetails?.Records) {
      setTableData(jawdaLevelDetails?.Records);
    }
  }, [jawdaLevelDetails]);
  ...
  handleSubmit = async () => {
    const rowsWithOrder = reorderedList.map((item, index) => ({
      jawdaLevelId: item.JAWDALevelId,
