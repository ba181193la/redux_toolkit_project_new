...
const columns = [
  ...
  { id: 'IncidentDepartment', name: 'Incident Department' },
];
...
export const IncidentOccuringDepartmentsChart = ({ 
    data = [], 
    selectedFacility, 
    selectedModuleId, 
    selectedMenu, 
    ...
    const groupedData = Object.values(
      data.reduce((acc, item) => {
        ...
        acc[item.DepartmentId].Count += item.Count;
        return acc;
      }, {})
    ...
    const options = {
      ...commonOptions,
      xaxis: { 
        categories: groupedData.map(item => item.DepartmentName || "UNKNOWN"),
        labels: { 
          show: true, 
        //   rotate: -45, 
        //   rotateAlways: true, 
        ...
        events: {
            dataPointSelection: async (event, chartContext, config) => {
              ...
              if (selectedIncident) {
                ...
                setSelectedId(selectedIncident.DepartmentId);
                dispatch(setTableFilters({ ...tableFilters, pageIndex: 0, selectedGraphType: "Department", customProperty: selectedIncident.DepartmentId }));
