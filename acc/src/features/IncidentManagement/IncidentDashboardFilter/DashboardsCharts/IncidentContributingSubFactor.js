...
export const IncidentContributingSubFactorChart = ({
  data = [],
  selectedFacility,
  selectedModuleId,
  selectedMenu,
  ...
  const labels = sortedData.map((item) => item.SubFactorName);
    ...
    chart: {
      ...
      events: {
              dataPointSelection: (event, chartContext, config) => {
                ...
                if (selectedIncident) {
                  setSelectedId(null);
                  setSelectedId(selectedIncident.SubFactorId);
                  dispatch(
                    setTableFilters({
                      ...tableFilters,
                      pageIndex: 0,
                      selectedGraphType: 'Actionclosure',
                      customProperty: selectedIncident.SubFactorId,
                    })
                  );
                }
              },
