...
import RejectedData from './InvestigationRejectedData';
import DataTableRejected from './DataTableRejected';
import FilterFormFields from './FilterFormRejectedFields';
import {
  FlexContainer,
  StyledButton,
  StyledImage,
...
const InvestigationRejected = () => {
  const [seletedRejectedColumns, setSelectedRejctedColumns] =
    useState(RejectedData);
...
const [color, setColor] = useState('');
 ...
 const [triggerDownloadData] = useGetDownloadDataInvestigationRejectMutation();
    ...
    const handleOnDownloadOrPrint = async (fileType) => {
      if (filteredRecords?.length > 0) {
        try {
          ...
          if (fileType === 'Pdf') {
            saveAs(blob, 'Incident Investigation Rejected.pdf');
            setMessage('Download completed');
            setTimeout(() => {
              setIsLoading(false);
            }, 2000);
          } else {
            saveAs(blob, 'Incident Investigation Rejected.xlsx');
            setMessage('Download completed');
            setTimeout(() => {
              setIsLoading(false);
            }, 2000);
  ...
  const {
    data: getIncidentInvestigationRejected = [],
    isFetching,
    refetch,
  } = useGetIncidentInvestigationRejectedQuery(
    {
  ...
  const { totalRecords, records } = getIncidentInvestigationRejected || {
    totalRecords: 0,
    records: [],
  };
  ...
  const alteredLabels = {
    Data: labels.filter((label) =>
      fieldLabels.includes(label.FieldEnglishLabel)
    ),
  };
...
export default InvestigationRejected;
