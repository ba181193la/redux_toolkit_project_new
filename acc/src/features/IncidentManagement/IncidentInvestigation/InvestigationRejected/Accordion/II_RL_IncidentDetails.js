...
import VisitorTable from './Datatable/II_Rejected_VisitorTable';
import WitnessTable from './Datatable/II_Rejected_WitnessedBy';
        ...
        <Grid container spacing={2} p={0}>
          {configs.fieldsConfig.map((fieldConfig) => {
            ...
            const getFormattedValue = (value, name) => {
              if ((name === 'IncidentDate' || name === 'ReportDate') && value) {
                return formatDate(value);
              }
              if ((name === 'IncidentTime' || name === 'ReportTime') && value) {
