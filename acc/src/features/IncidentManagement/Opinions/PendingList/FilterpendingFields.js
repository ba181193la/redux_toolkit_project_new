...
const FilterPendingFields = ({ handleToggleFilter, searchLabels }) => {
  ...
  const {
    ...
    requestedUserId,
    facilityId,
  } = useSelector((state) => state.opinion.pendingFilters);

  const requestedBYId=selectedRowData?.UserId
  ...
  const initialValues = {
    ...
    requestedUserId: requestedUserId || 0,
  };
  const fieldsConfig = [
    {
    ...
    },
    {
      ...
      component: 'Dropdown',
      name: 'incidentNumber',
      options: data?.Data?.Result?.IncidentNumberPending?.map((incidentNo) => ({
          ...
          <Grid container spacing={2} display={'flex'}>
            ...
            >
              ...
              <StyledButton
                ...
                // onClick={() => {
                //   dispatch(resetPendingFilters());
                //   setSelectedRowData(null); // Reset state first
                //   setTimeout(() => {
                //     resetForm();
                //   }, 0); // Allow state update before form reset
                // }}
              >
                {t('ClearAll')}
              </StyledButton>
...
export default FilterPendingFields;
