...
const IncidentApprovalDetails = () => {
  ...
  const [approvedDateTime, setApprovedDateTime] = useState(null);

  useEffect(() => {
      const ApprovedDateTimeValue = incidentApprovalSafe['ApprovedDateTime'];
      if (ApprovedDateTimeValue) {
        setApprovedDateTime(ApprovedDateTimeValue);
      }
  ...
  const fieldsConfig = [
    ...
    {
      fieldId: `IA_P_ApprovalStatus`,
      translationId: 'IM_IA_ApprovalStatus',
      label: 'Status',
      name: 'ApproverStatus',
    },
    {
      fieldId: `IA_P_Remarks`,
      ...
      if (fields?.length > 0) {
        const matchingSections = fields[0]?.Sections?.filter(
          (section) =>
            ...
            section.SectionName ===
              'Accordion-Re-Assigned Investigator(s) History' ||
            section.SectionName === 'Accordion-Request Information History' ||
            section.SectionName ===
              'Accordion-Re-Assigned Opinion Respondent(s) History'
        );
    ...
    >
      ...
      >
        ...
        <AccordionDetails>
            ...
            <Grid container spacing={2} p={0} mb={5}>
              {configs.fieldsConfig?.map((fieldConfig) => {
                ...
                const getFormattedValue = (value, name) => {
                  if (name === 'ApprovedDateTime') {
                    if (value) {
                   return formatDate(value);
              }

                  }
                  if (name === 'ApprovedTime' && approvedDateTime) {
                    const timePart = approvedDateTime?.split('T')[1];
                ...
                const getFieldStyle = (fieldName, fieldValue) => {
                  if (fieldName === 'ApproverStatus' && fieldValue !== '') {
                ...
                return (
                  <Grid item xs={4} key={fieldConfig.fieldId}>
                    ...
                    <StyledTypography style={getFieldStyle(fieldConfig.name, fieldValue)}>
          ...
          {incidentApprovalSafe?.assignedInvestigatorList?.length > 0 && (
          <>
            <FormLabel style={{ marginBottom: '10px', marginTop: '10px' }}>
              {getlabel(
                'IM_IA_AssignedInvestigator(s)',
                filterLabels,
                i18n.language
              )}
            </FormLabel>
 ...
 {incidentApprovalSafe?.opinionsExchangedConfig?.length > 0 && (
          <>
          <FormLabel style={{ marginBottom: '10px', marginTop: '10px' }}>
...
{incidentApprovalSafe?.rejectionHistoryConfig?.length > 0 && (
          <>
          <FormLabel style={{ marginBottom: '10px', marginTop: '10px' }}>
        ...
        {incidentApprovalSafe?.reAssignedInvestigatorHistoryConfig?.length > 0 && (
          <>
          <FormLabel style={{ marginBottom: '10px', marginTop: '10px' }}>
            {getlabel(
              'IM_IA_Re-AssignedInvestigator(s)History',
              filterLabels,
              i18n.language
            )}
          </FormLabel>
        ...
        {incidentApprovalSafe?.requestedHistory?.length > 0 && (
          <>

          <FormLabel style={{ marginBottom: '10px', marginTop: '10px' }}>
            {getlabel(
              'IM_IA_RequestInformationHistory',
              filterLabels,
              i18n.language
            )}
          </FormLabel>
        ...
        {incidentApprovalSafe?.reAssignedOpinionrespondentsConfig?.length > 0 && (
          <>
          <FormLabel style={{ marginBottom: '10px', marginTop: '10px' }}>
            {getlabel(
              'IM_IA_Re-AssignedOpinionRespondent(s)History',
              filterLabels,
              i18n.language
            )}
          </FormLabel>
