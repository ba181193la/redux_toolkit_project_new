...
import AddEditVerificationStatus from './AddEditVerificationStatus';

const VerificationStatus = ({ labels, fieldAccess }) => {
  //* Hooks declaration
  ...
  const { pageIndex, pageSize } = useSelector(
    (state) => state?.staffDemography?.verificationStatus
  );
  ...
  const handleOnPageChange = (event, newPage) => {
    dispatch(setVerificationStatusPageIndex(newPage));
  };

  const handleOnPageSizeChange = (event) => {
    dispatch(setVerificationStatusPageSize(parseInt(event?.target?.value, 10)));
    dispatch(setVerificationStatusPageIndex(0));
  };
  ...
  const orderedStatusList = [...statusList].sort((a, b) => {
  ...
  const tableColumnList = [
    {
      id: 'Facility',
      translationId: 'MM_SF_VS_Facility',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_VS_Facility')?.IsShow,
    },
    {
      id: 'VerificationStatus',
      translationId: 'MM_SF_VS_StaffDemographyVerificationStatus',
    ...
    {
      id: 'AddedEditedBy',
      translationId: 'MM_SF_VS_AddedEditedBy',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_VS_AddedEditedBy')
        ?.IsShow,
    },
    {
      id: 'AddedEditedDate',
      translationId: 'MM_SF_VS_AddedEditedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_VS_AddedEditedDate')
        ?.IsShow,
    },
  ];
    ...
    const callback = async () => {
      try {
        ...
        showSweetAlert({
          type: 'success',
          title: 'Deleted!',
          text: 'Verification Status has been deleted.',
          timer: 3000,
          gif: SuccessGif,
          height: 250,
          width: 250,
        ...
        >
          ...
          >
            ...
            >
              <Label
                value={getlabel(
                  fieldLabels?.find(
                    (x) => x.TranslationId === 'MM_SF_VS_Facility'
                  )?.TranslationId,
                  {
                    Data: fieldLabels,
                    Status: labels?.Status,
          ...
          <FlexContainer flexDirection="column" width="100%" padding="10px">
            ...
            >
              <CustomScrollbars
                style={{
                  height: orderedStatusList?.length > 0 ? '350px' : '85px',
                }}
              ...
              >
                {false ? (
                  ...
                  <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
                    ...
                    {orderedStatusList?.length > 0 ? (
                      <TableBody>
                        {orderedStatusList?.map((row, rowIndex) => (
                                  ...
                                  {checkAccess(
                                    ...
                                    <Tooltip title="Delete" arrow>
                                      <StyledImage
                                        ...
                                        src={DeleteIcon}
                                        alt="Delete"
                                        onClick={() => deleteStatus(row?.TabId)}
                                      />
                                    </Tooltip>
                                  )}
                                </FlexContainer>
