...
const LicenseDataTable = ({
  ...
  const educationColumnsList = [
    ...
    {
      id: 'authority',
      translationId: 'MM_SF_IssueAuthority',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_IssueAuthority')
        ?.IsShow,
    },
    {
      id: 'country',
      translationId: 'MM_SF_Country',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_Country')?.IsShow,
    },
    {
      id: 'fromDate',
      translationId: 'MM_SF_IssueDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_IssueDate')?.IsShow,
    },
    {
      id: 'toDate',
      translationId: 'MM_SF_ExpiryDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_ExpiryDate')?.IsShow,
    },
    {
      id: 'certificate',
    ...
    },
    {
      ...
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_SelectLatest')
        ?.IsShow,
    },
    {
      id: 'submittedBy',
      translationId: 'MM_SF_SubmittedBy',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_SubmittedBy')?.IsShow,
    },
    {
      id: 'submittedDate',
      translationId: 'MM_SF_SubmittedDate',
      isShow: pageFields?.find((x) => x.FieldId === 'SF_L_SubmittedDate')
        ?.IsShow,
    },
  ];
  ...
  //* Add more data
  const addMore = () => {
    const lastRecord = licenseRecords[licenseRecords?.length - 1];
    const unsavedRecord = licenseRecords?.findIndex(
    ...
    if (
      ...
      showToastAlert({
        type: 'custom_info',
        text: `Enter records in Exoerience Tab row no ${licenseRecords?.length}`,
        gif: 'InfoGif',
      });
  ...
  const changeRowDetails = (rowIndex, column, value) => {
    const updatedRecords = [...licenseRecords];
    updatedRecords[rowIndex] = { ...updatedRecords[rowIndex], [column]: value };
    setLicenseRecords(updatedRecords);
  };

  //* Delete row details
  const deleteRecord = (index) => {
    const records = [...licenseRecords];
    setLicenseRecords(records.filter((e, i) => i !== index));
  };
  ...
  return (
    <FlexContainer flexDirection="column" width="100%">
      <TableContainer component={Paper} style={{ border: '1px solid #03a78e' }}>
        <Table stickyHeader style={{ borderCollapse: 'collapse' }}>
          <StyledTableHead backgroundColor="#03a78e" borderLeft="none">
          ...
          {licenseRecords?.length > 0 ? (
            <TableBody>
              {licenseRecords?.map((row, rowIndex) => (
