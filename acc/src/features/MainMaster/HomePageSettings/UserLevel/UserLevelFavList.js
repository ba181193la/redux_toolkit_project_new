...
import UserLvelMenuTable from './UserLevelMenuTable';
...
const UserLevelMenuList = () => {
  const navigate = useNavigate();
  const { i18n, t } = useTranslation();
  const dispatch = useDispatch();
  const [menuList, setMenuList] = useState([])
  const [parentMenu, setParentMenu] = useState([])
  const { userDetails, menuDetails } = useSelector((state) => state.auth);

  const { isMobile } = useWindowDimension();


  const { userLevelMenu } = useSelector((state) => state.homePageSetting)
  ...
  useEffect(() => {
    if (menuDetails?.length > 0) {
      ...
      const menuListData = userLevelMenuList?.map((menuItem) => {
        ...
        if (menuItem.ModuleId === parentMenu?.ModuleId && menuItem.MenuId !== parentMenu?.MenuId) {
          return {
            ...menuItem,
      ...
      dispatch(setUserLevelMenu(menuListData))

    }

  }, [menuDetails, userLevelMenuList])

  const [triggerCreatedMenuList,{isLoading}] = useCreateUserLevelMenuListMutation();



  const handleSubmit = async () => {
    try {
      const payload = userLevelMenu?.filter((item) => item.IsChecked).map((data) => ({
        menuId: data.MenuId,
...
export default UserLevelMenuList;
