...
import { Divider, Tooltip, List, ListItem, ListItemIcon } from '@mui/material';
...
const AddNewRole = () => {
  ...
  const { data: { organizationLevelMenuList } = {}, isFetching: isFetchingMenuList } = useGetOrganizationLevelMenuListQuery(
    {
  ...
  useEffect(() => {
    if (menuDetails?.length > 0) {
      ...
      const menuListData = organizationLevelMenuList?.map((menuItem) => {
  ...
  }, [menuDetails, organizationLevelMenuList])
