...
import { setFilters } from '../../../redux/features/mainMaster/mailLogSlice';
import formatDate from '../../../utils/FormatDate';

const DataTable = ({ columns = [], mailLogData, totalRecords, labels }) => {
  const { i18n, t } = useTranslation();
  const dispatch = useDispatch();

  const { pageSize, pageIndex } = useSelector((state) => state.mailLog.filters);
  const [order, setOrder] = useState('asc');
  const [orderBy, setOrderBy] = useState('');

  const filters = useSelector((state) => state.mailLog.filters);
  ...
  const sortedRecords = [...mailLogData].sort((a, b) => {
                      ...
                      {columns
                        ...
                        .map((column) => {
                          return (
                            ...
                            >
                              {column.id === 'CreatedDate' && row[column.id]
